...
import 'package:foodly_user/common/utils/security.dart';
...
class ChatController extends GetxController{
  ...
  void onInit(){
    ...
    doc_id = Uri.decodeComponent(Get.parameters['doc_id'] ?? '');
    state.to_uid.value = Uri.decodeComponent(Get.parameters['to_uid'] ?? '');
    state.to_avatar.value = Uri.decodeComponent(Get.parameters['to_name'] ?? '');
  //  final toAvatar = Uri.decodeComponent(Get.parameters['to_avatar'] ?? '');

    if (doc_id.isNotEmpty && state.to_uid.value.isNotEmpty) {
     // loadData(docId, toUid, toName, toAvatar);
