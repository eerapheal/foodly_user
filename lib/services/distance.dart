...
class DistanceTime {
  ...
  DistanceTime({required this.distance, required this.time, required this.price});
}
...
class Distance {
  DistanceTime calculateDistanceTimePrice(double lat1, double lon1, double lat2,
    ...
    var distance = earthRadiusKm * c; // Correct calculation of distance

    // Calculate time (distance / speed)
    var time = distance / speedKmPerHr;

    // Calculate price (distance * rate per km)
    var price = distance * pricePerKm+5;
