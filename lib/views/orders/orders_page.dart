   ...
   // final orderController = Get.put(OrderController());
    ...
    return Obx(() => orderController.paymentUrl.contains("https")
                                ...
                                if (distanceTime.distance > 10.0) {
                                  ...
                                  /*orderController.createOrder(orderData, order,
                                        ...
                                        if (paymentSuccess) {
                                          ...
                                          LoyaltyTransaction loyalty = LoyaltyTransaction(
                                              userId: controller.defaultAddress!.userId,
                                              points: 5,
                                              reason: TransactionReason.earn);
                                          orderController.updateLoyaltyPoints(loyalty);
                                        } else {
                                          showCustomSnackBar(
        ...
        bottomNavigationBar: Container(
          ...
          child: Obx(() {
            final checkoutController = Get.find<CartCheckoutController>();
              return GestureDetector(
                onTap: () {
                  /*print("Items in the card ${items.length}");
                  if(checkoutController.moreItems.value==true){
                    showCustomSnackBar("Just select a single restaurant to place order. First remove other items from that's name from the same restaurant", isError: false, title: "Warning", duration: Duration(seconds: 5));
                  }else {
                    final controller = Get.put(OrderController());
                    controller.setUserCart(items[0]);
